...
having issues. Exception: ' + errorToReport + '. Go to: http://newrelic.com \
for more details.';
};
...
module.exports.notifyOnError = function(appError, request, response, next) {
  fs.readFile('./config/administrators.json', 'utf8', function(readErr, data) {
    if (readErr) {
      console.error('Could not read administrators file: ' + readErr.toString());
    } else {
      var admins = JSON.parse(data);

      _.each(admins, function(admin) {
        twilioClient.sendSms(admin.phoneNumber, formatMessage(appError.message));
      });
    }
  });
  next();
